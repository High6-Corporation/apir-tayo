...
const benefits: Benefit[] = [
  {
    ...
    description:
      "Make your brand look professional and trustworthy from the moment visitors land on your site.",
  },
  {
    icon: "/assets/clear-services-icon.svg",
    title: "Clear services with easy inquiry or booking",
    description:
      "Help visitors quickly understand what you offer and take action without confusion.",
  },
  {
    icon: "/assets/faster-launch-icon.svg",
    title: "Faster launch at lower cost—without overwhelm",
    description:
      "Get your website live quickly and affordably, without the stress of building everything from scratch.",
  },
];

export function WhyOnePageSection({
  whyOnePageParagraph,
}: WhyOnePageSectionProps) {
  const { elementRef: headerRef, isVisible: headerVisible } =
    useScrollAnimation();
  const { elementRef: benefitsRef, isVisible: benefitsVisible } =
    useScrollAnimation();
  const { elementRef: taglineRef, isVisible: taglineVisible } =
    useScrollAnimation();
    ...
    >
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-20">
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 mb-12">
          ...
          <div
            ref={headerRef}
            className={`flex flex-col max-w-[535px] max-[1023px]:max-w-none max-[1023px]:items-center max-[1023px]:text-center scroll-animate-left ${headerVisible ? "scroll-animate-visible" : ""}`}
          >
            {/* Section Header */}
            <div className="flex items-center gap-1 mb-2">
              <img
                src="/assets/why-one-page-icon.svg"
                alt=""
            ...
            <h3 className="font-semibold tracking-[-0.6px] mb-6">
              <span className="text-[#333]">A Single Page.</span>
              <br></br>
            ...
            {/* CTA Button */}
            <Link href="/contact">
              <PrimaryButton className="self-start max-[1023px]:self-center">
                Get Started Today
              </PrimaryButton>
            </Link>
          </div>
          ...
          <div ref={benefitsRef} className="flex flex-col gap-4">
            {benefits.map((benefit, index) => (
              <div
                key={index}
                className={`bg-white border border-[rgba(0,0,0,0.05)] rounded-[20px] shadow-[0px_4px_25px_0px_rgba(0,0,0,0.05)] p-7 flex gap-[14px] items-start scroll-animate ${benefitsVisible ? "scroll-animate-visible" : ""} stagger-${index + 1}`}
              >
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <p className="font-medium text-[15px] leading-[28px] text-[#59646b]">
                    {benefit.description}
                  </p>
                </div>
              </div>
        ...
        <div
          ref={taglineRef}
          className={`text-center scroll-animate ${taglineVisible ? "scroll-animate-visible" : ""}`}
        >
          <h3 className="font-semibold text-[#333]">
            <span>One page. One clear message. </span>
            <span className="text-[#5757ff]">
              One big upgrade for your brand.
            </span>
          </h3>
        </div>
      </div>
